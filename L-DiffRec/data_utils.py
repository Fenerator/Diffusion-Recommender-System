...
def data_load(train_path, valid_path, test_path):
    ...
    train_data = sp.csr_matrix(
        (np.ones_like(train_list[:, 0]), (train_list[:, 0], train_list[:, 1])),
        dtype="float32",
        shape=(n_user, n_item),
    )

    valid_y_data = sp.csr_matrix(
        (np.ones_like(valid_list[:, 0]), (valid_list[:, 0], valid_list[:, 1])),
        dtype="float32",
        shape=(n_user, n_item),
    )  # valid_groundtruth

    test_y_data = sp.csr_matrix(
        (np.ones_like(test_list[:, 0]), (test_list[:, 0], test_list[:, 1])),
        dtype="float32",
        shape=(n_user, n_item),
    )  # test_groundtruth
...
class SubData(Dataset):
    def __init__(self, train_path, valid_path, test_path, num_sub=1000):
        ...
        (
            ...
            self.num_user,
            self.num_item,
        ) = subdata_load(train_path, valid_path, test_path)
            ...
            try:
                m = np.random.choice(
                    np.array(
                        list(
                            self.item_set
                            - set(self.train_dict[uid])
                            - set(self.valid_dict[uid])
                        )
                    ),
                    self.num_sub - len(test_list[uid]),
                    replace=False,
                )
            except:
                m = np.random.choice(
                    np.array(list(self.item_set - set(self.train_dict[uid]))),
                    self.num_sub - len(test_list[uid]),
                    replace=False,
                )
